...
export function ProcessingPage({ projectId }: { projectId: string }) {
  ...
  const [activeStage, setActiveStage] =
    useState<BrowserProcessingStage>("CLASSIFY_DOCUMENTS");
  ...
  const start = useCallback(async () => {
    ...
    if (!preflight.valid) {
      throw new Error(
        `PREFLIGHT_BLOCKED: ${preflight.blockingReasons.join(" ")}`
      );
    }
    ...
    const run: BrowserProcessingRun = {
      ...
      checkpoint: {
        ...
        completedStages: [],
        processedPages: 0,
        totalPages: documentsToProcess.reduce(
    ...
    worker.onmessage = (event: MessageEvent<BrowserProcessingEvent>) => {
      ...
      if (message.type === "COMPLETE") {
        void (async () => {
          const currentDocuments = await service.listDocuments(projectId);
          const workerResult =
            incrementalRef.current && previousAnalysisRef.current
              ? (() => {
                  ...
                  return {
                    ...
                    supplierLines: [
                      ...previous.supplierLines,
                      ...message.result.supplierLines
                    ],
                    ...
                    diagnostics: {
                      ...
                      pagesParsed:
                        previous.diagnostics.pagesParsed +
                        message.result.diagnostics.pagesParsed,
                      ocrRequiredPages:
            ...
            if (current) {
              await persistRun({
                ...
                warnings: [
                  ...
                  "Keine Basis-Positionen erkannt."
                ],
                updatedAt: new Date().toISOString()
              });
            }
          ...
          const snapshot = buildBrowserAnalysis({
            ...
            documents: currentDocuments.filter((document) =>
              relevantDocuments.some(
                (relevant) => relevant.documentId === document.documentId
              )
            ),
            result: workerResult
          });
  ...
  useEffect(() => {
    ...
    void Promise.all([
      service.getProject(projectId),
      service.currentProcessingRun(projectId)
    ]).then(([storedProject, current]) => {
      if (cancelled) return;
    ...
    return (
      <div className="browser-workflow-page processing-page" data-processing-interrupted>
        <section className="processing-interrupted">
          ...
          <div>
            <button onClick={() => void start()}><RotateCcw size={17} /> Fortsetzen</button>
    ...
    <div className="browser-workflow-page processing-page" data-processing-page>
      ...
      <div className="processing-layout">
          ...
          {visibleStages.map((stage) => {
            ...
            return (
              <div className={completed ? "completed" : active ? "active" : ""} key={stage}>
                <span>
                  {completed ? <Check size={17} /> : active ? <LoaderCircle className="spin" size={17} /> : <Circle size={13} />}
                </span>
        ...
        <aside className="processing-detail">
          ...
          <dl>
            <div><dt>Dokument</dt><dd>{currentDocument || "Vorbereitung"}</dd></div>
            <div><dt>Seite</dt><dd>{currentPage ?? "—"}</dd></div>
            <div><dt>Verarbeitete Seiten</dt><dd>{processedPages} von {totalPages}</dd></div>
            <div><dt>Basis-Positionen</dt><dd>{project?.basisPositionCount ?? 0}</dd></div>
          </dl>
          {warnings.length ? (
